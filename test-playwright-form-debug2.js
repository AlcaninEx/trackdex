...
(async () => {
  ...
  try {
    ...
    // Try to find user-id input with different selectors
    console.log('\n🔍 Buscando inputs de user-id...');
    
    // Try multiple selectors
    const selectors = [
      '#new-community-user-id',
      '#create-community-form #new-community-user-id',
      '.community-form #new-community-user-id',
      'input[id="new-community-user-id"]'
    ];
    ...
    for (const sel of selectors) {
      ...
      if (el) {
        ...
        if (visible) {
          await el.fill(`debuguser_${Date.now()}`);
          break;
        }
      }
    }
    
    // Also check all inputs in create form
    const allInputs = await page.$$eval('#create-community-form input', inputs => inputs.map(i => ({ id: i.id, type: i.type, visible: i.offsetWidth > 0, value: i.value })));
